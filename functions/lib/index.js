...
export const hermesPropose = onCall({ secrets: [HERMES_API_KEY, CF_ACCESS_CLIENT_ID, CF_ACCESS_CLIENT_SECRET], cors: true, timeoutSeconds: 60 }, async (req) => {
    if (!req.auth)
        throw new HttpsError('unauthenticated', 'Sign in to use Hermes.');
    const data = req.data;
    if (!data?.song)
        throw new HttpsError('invalid-argument', 'Missing song context.');
...
async function callHermes(messages) {
    ...
    if (HERMES_API_KEY.value())
        headers['Authorization'] = `Bearer ${HERMES_API_KEY.value()}`;
    if (CF_ACCESS_CLIENT_ID.value())
        headers['CF-Access-Client-Id'] = CF_ACCESS_CLIENT_ID.value();
    if (CF_ACCESS_CLIENT_SECRET.value())
        headers['CF-Access-Client-Secret'] = CF_ACCESS_CLIENT_SECRET.value();
    let res;
    try {
        ...
        throw new HttpsError('unavailable', `Could not reach Hermes: ${e.message}`);
    }
    ...
    if (!res.ok)
        throw new HttpsError('internal', `Hermes returned HTTP ${res.status}.`);
    const body = (await res.json());
    const content = body.choices?.[0]?.message?.content ?? body.content ?? body.text ?? '';
    if (!content.trim())
        throw new HttpsError('internal', 'Hermes returned an empty reply.');
    return content.trim();
}
function buildMessages(d) {
    const system = 'You are Hermes, a songwriting co-writer inside Tig Music. Offer ONE short, singable ' +
    ...
    const ctx = `Song "${d.song.title}", key of ${d.song.keyName}, ${d.song.tempo} bpm.` +
